...
import { Goal, GoalSummary as GoalSummary } from '@app/shared/interfaces';
...
export class MonthComponent implements OnInit, OnChanges {
  ...
  @Input() dailySummaries: Record<string, GoalSummary> = {};
  ...
  private _buildWeek(start: Date, month: number): DayDate[] {
    ...
    for (var i = 0; i < 7; i++) {
      ...
      const goals = this.dailySummaries[dailyGoalKey]?.goals || [];
      days.push({
        date,
        month,
        goals,
